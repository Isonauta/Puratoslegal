    ...
    <>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
          <h1 className="text-xl font-bold">{permit.taskDescription}</h1>
            ...
            <div className="grid grid-cols-2 gap-3">
              {permit.signatures.map((sig) => (
                <div key={sig.id} className="space-y-1">
                  <p className="text-xs text-gray-500">{SIGNATURE_ROLE_LABELS[sig.role] ?? sig.role}</p>
                  <p className="text-sm font-medium">{sig.user?.name ?? "—"}</p>
                  {sig.imageData && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={sig.imageData} alt="Firma" className="border rounded-lg bg-white h-20 object-contain" />
                  )}
                </div>
              ))}
            </div>
